...
import { generateProof } from "../sdk/zktanitid";

export default function ProofOptions({ kind, credential }: { kind: string, credential: any }) {
  ...
  return (
    <div style={{ padding: 12, margin: "12px 0", borderRadius: 12, border: "1px solid #eaeaea" }}>
      <h3 style={{ marginTop: 0 }}>Generate <code>{kind}</code> Proof</h3>
      <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <label>
          <input type="checkbox" checked={revealExtra} onChange={e => setRevealExtra(e.target.checked)} />
      ...
      {proof && (
        <details open style={{ marginTop: 8 }}>
          ...
          <pre style={{ overflowX: "auto" }}>{JSON.stringify(proof, null, 2)}</pre>
        </details>
      )}
    </div>
  );
